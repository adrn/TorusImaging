...
def get_mw_potential(mdisk):
    ...
    def objfunc(ln_mhalo):
        ...
        tmp_mw = gp.MilkyWayPotential(disk=dict(m=mdisk), halo=dict(m=mhalo))
        test_v = tmp_mw.circular_velocity(
            [-rsun.to_value(u.kpc), 0, 0] * u.kpc
        ).to_value(u.km / u.s)[0]
        return (vcirc.to_value(u.km / u.s) - test_v) ** 2

    minit = potentials["1.0"]["halo"].parameters["m"].to_value(u.Msun)
    res = minimize(objfunc, x0=np.log(minit), method="powell")

    if not res.success:
        return np.nan

    mhalo = np.exp(res.x)
    return gp.MilkyWayPotential(disk=dict(m=mdisk), halo=dict(m=mhalo))
