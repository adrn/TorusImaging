...
class Dataset:
    ...
    def __init__(self, filename_or_tbl):
        if isinstance(filename_or_tbl, str) or isinstance(
            filename_or_tbl, pathlib.Path
        ):
        ...
        for col in self.t.colnames:
            if (
                col.upper().endswith("_FE")
                or col.upper().startswith("FE_")
                or col.upper().endswith("_H")
            ) and not col.upper().startswith("FLAG"):
    ...
    def elem_ratios(self):
        if not hasattr(self, "_elem_ratios"):
            self._elem_ratios = ["FE_H"] + sorted(
                [
                    x
                    for x in self.t.colnames
                    if x.endswith("_FE")
                    and not x.startswith("E_")
                    and not x.startswith("FLAG_")
                    and not x.startswith("CHI_")
                    and not x.startswith("FLUX_")
                    and not x.startswith("NR_")
                ]
            )
    ...
    def get_elem_ratio(self, elem1, elem2=None):
        ...
        if elem2 is None:
            try:
                ...
                raise RuntimeError(
                    "If passing a single elem ratio string, "
        ...
        if elem2 == "H":
            ...
            return self.t[self.elem_ratios[i1]] - self.t[self.elem_ratios[i2]]
...
class GALAHDataset(Dataset):
    ...
    def _init_mask(self):
        quality_mask = (self.t["flag_sp"] == 0) & (self.t["flag_fe_h"] == 0)
    ...
    def get_mh_am_mask(self, low_alpha=True):
        ...
        low_alpha_mask = mh_alpham_path.contains_points(
            np.stack((np.array(self.t["FE_H"]), np.array(self.t["ALPHA_FE"]))).T
        )
