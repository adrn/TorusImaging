...
def _same_actions_objfunc_staeckel(p, pos, vy, potential_name, match_actions):
    vx, vz = p
    w0 = gd.PhaseSpacePosition(pos=pos, vel=[vx, vy, vz] * u.km / u.s)

    o = potentials[potential_name].integrate_orbit(
        w0, dt=1.0 * u.Myr, t1=0, t2=1 * u.Gyr, Integrator=gi.DOPRI853Integrator
    )
    aaf = get_staeckel_aaf(o[::2], galpy_potentials[potential_name])
    ...
    for n in range(fiducial_w0.shape[0]):
        if staeckel:
            o = potentials["1.0"].integrate_orbit(
                fiducial_w0[n], dt=0.5, t1=0, t2=2 * u.Gyr
            )  # MAGIC NUMBERS
